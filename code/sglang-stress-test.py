...
async def send_request(session, url, prompt, request_id, progress_bar, task_id, max_tokens=DEFAULT_MAX_TOKENS):
...
async def run_stress_test(url, prompt, concurrent_requests, total_requests, max_tokens=DEFAULT_MAX_TOKENS):
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            for i in range(total_requests):
                tasks.append(send_request(
                    session, url, prompt, i+1, progress, task_id, max_tokens=max_tokens
                ))
...
def print_results(results, test_info, concurrent_requests, total_requests, url, max_tokens):
    ...
    successful_requests = [r for r in results if isinstance(r.get("status"), int) and r.get("status") == 200]
    ...
    stats_table = Table(title=f"SGLang Stress Test Results - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    stats_table.add_row("Successful Requests", f"{len(successful_requests)} ({len(successful_requests)/total_requests*100:.1f}%)")
    stats_table.add_row("Failed Requests", f"{len(failed_requests)} ({len(failed_requests)/total_requests*100:.1f}%)")
    stats_table.add_row("Total Test Duration", f"{test_info['overall_duration']:.2f} seconds")
    
    if response_times:
        stats_table.add_row("Min Response Time", f"{min(response_times):.2f} seconds")
        stats_table.add_row("Max Response Time", f"{max(response_times):.2f} seconds")
        stats_table.add_row("Average Response Time", f"{statistics.mean(response_times):.2f} seconds")
        stats_table.add_row("Median Response Time", f"{statistics.median(response_times):.2f} seconds")
        stats_table.add_row("P90 Response Time", f"{response_percentiles['p90']:.2f} seconds")
        stats_table.add_row("P95 Response Time", f"{response_percentiles['p95']:.2f} seconds")
        stats_table.add_row("P99 Response Time", f"{response_percentiles['p99']:.2f} seconds")
        
        if len(response_times) > 1:
            stats_table.add_row("Std Dev Response Time", f"{statistics.stdev(response_times):.2f} seconds")
        ...
        actual_throughput = len(successful_requests) / test_info["overall_duration"]
        
        stats_table.add_row("Theoretical Max Throughput", f"{traditional_throughput:.2f} requests/second")
        stats_table.add_row("Actual Throughput", f"{actual_throughput:.2f} requests/second")
        ...
        stats_table.add_row("Avg Tokens Per Request", f"{avg_tokens_per_request:.2f}")
        
        # Peak requests in flight
        all_times = [(r["start_time"], "start") for r in results] + [(r["end_time"], "end") for r in results]
        all_times.sort()
...
def main():
    ...
    parser.add_argument("--url", default=DEFAULT_URL, help=f"SGLang API URL (default: {DEFAULT_URL})")
    parser.add_argument("--prompt", default=DEFAULT_PROMPT, help="Prompt to send (default is a short explanation request)")
    parser.add_argument("--concurrent", type=int, default=DEFAULT_CONCURRENT, help=f"Number of concurrent requests (default: {DEFAULT_CONCURRENT})")
    parser.add_argument("--total", type=int, default=DEFAULT_TOTAL, help=f"Total number of requests to send (default: {DEFAULT_TOTAL})")
    ...
    console.print(f"Prompt: '{args.prompt[:50]}...' ({len(args.prompt)} chars)")
    ...
    try:
        ...
        console.print(f"[bold green]Test completed in {total_time:.2f} seconds[/bold green]")
