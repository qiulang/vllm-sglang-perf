#!/usr/bin/env python3
"""
vLLM Stress Test Script with Improved Metrics

This script performs stress testing on vLLM by sending multiple concurrent requests
and measuring response times and throughput with more accurate calculations.
"""
    ...
    try:
        async with session.post(url, json=data, timeout=DEFAULT_TIMEOUT) as response:
            ...
            if response.status == 200:
                ...
                # Extract tokens from vLLM response
...
def print_results(results, test_info, concurrent_requests, total_requests, model, url, max_tokens, temperature):
    ...
    stats_table = Table(
        title=f"vLLM Stress Test Results - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    parser = argparse.ArgumentParser(
        description="vLLM Stress Test Tool with Improved Metrics")
    ...
    console.print(f"[bold green]Starting vLLM Stress Test[/bold green]")
